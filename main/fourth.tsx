...
type FourthProps = { onTotalsChange?: (t: Totals) => void };

const Fourth: React.FC<FourthProps> = ({ onTotalsChange }) => {
  ...
  // ---------- Fourth model (adjust as needed) ----------
  // This example mirrors your latest "Third" style; change constants/formula if needed.
  const getResults = (c: { x: string; y: string; quantity: string }) => {
    ...
    const gr = x * 8 + y * 2 + (y / 3) * 4; // customize if needed
    const kv = x * y;
    const mek = quantity * 2; // customize if needed
    const basePrice = gr * 55 + kv * 52 + 700; // customize if needed
          ...
          <div key={c.id} className="mb-8">
            ...
            <div className="flex gap-4 mb-6">
              ...
              <div className="flex flex-col items-center">
                <Image
                  src="/6.png" // change to your desired image
                  alt="Image"
                  width={500}
                  height={300}
                  priority
            ...
            <div className="bg-gray-50 dark:bg-white p-4 rounded border dark:border-gray-200 dark:text-gray-900 w-full sm:w-[580px] min-w-[330px] ml-2">
              ...
              <div className="mb-2">
                კვ სრული: {(r.kv * quantity).toFixed(2)}, 1 ცლი - კვ:{" "}
                {r.kv.toFixed(2)}
              </div>
              <div className="mb-2">
